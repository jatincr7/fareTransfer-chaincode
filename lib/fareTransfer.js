...
class FareTransfer extends Contract {
    ...
    async EnrollCustomer(ctx, id, firstName, lastName, primaryTransit){
        const cus_exist = await this.CustomerExists(ctx, id);
        if(cus_exist){
    ...
    async UpdatePrimaryTransit(ctx, id, newTransitId) {
        ...
        let customerString = await this.GetCustomer(ctx, id);
        if (!customerString || customerString.length === 0) {
            throw new Error(`The customer ${id} does not exist`);
        }

        let customer = JSON.parse(customerString);

        // const _oldTransitId = customer.TransitId;
        customer.TransitId = newTransitId;
        console.log(customer);
        return await ctx.stub.putState(id, Buffer.from(JSON.stringify(customer)));
    }
